...
```powershell
.\scripts\check-version.ps1 -ExpectedTag v0.6.6
```
...
`publish-release.ps1` accepts either `0.6.6` or `v0.6.6` tags. It creates or repairs the GitHub Release, uploads the installer, signature, release notes through the release body, and `latest.json`, then verifies the public updater endpoint. Use `-Tag v0.6.6` when publishing a `v`-prefixed tag.
